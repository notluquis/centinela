...
The one sanctioned exception is the menu-bar count. `badgeMetric` lets someone make the bar number the count of unresolved (or for-review / escalating / regressed) *issues* instead of error *events*, because "147 error events across 3 unresolved issues" reads as if 144 went missing — they are different questions, occurrences versus open groups. The default is `errorEvents`, which rides the series already fetched and adds nothing to the cycle, so a fresh install stays cheap. Choosing an issue count is an opt-in that then persists: `refreshCheap()` pulls **one** issue-list read into the cycle for it — a deliberate, single, user-chosen extra request, not the door reopening on putting the whole expensive read on the timer. The count saturates at `maxIssues` and is shown as `N+` there rather than as a number that undercounts.
